...
/* Il faut ici délimite les fonctions du fichier assembleur en détectant les
directives .ent et .end qui indique le début et la function d'une fonction
Il faut donc parcourir toutes les lignes d'un programme et construire avec
chaque couple de ligne comportant les directives .ent FCT et .end FCT une
nouvelle fonction et l'ajouter à la liste des fonctions du programme

*/

/* NB : la méthode get_content() permet de récupérer une chaine de caractère
correspondant au contenu d'une ligne/directive/label/instruction. Par exemple :
...
void  Program::comput_CFG(){
   ...
   for(int i=0; i<size; i++){ // parcours des functions du programme
     ...
     cfg = new Cfg(current->get_firstBB(), current->nbr_BB ());
